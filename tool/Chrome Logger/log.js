...
(function() {
     ...
     * @var string
     */
    var HEADER_NAMES = ['hyman-data'];
    var HEADER_ENCRYPT = 'hyman-encrypt';
    ...
    function _decode(header,encryptType) {
        var data = atob(header);
        if('aes' == encryptType){
            data = _decodeAes(header);
        }
        return JSON.parse(data);
    }

    function _decodeAes(header){
        var key  = CryptoJS.enc.Utf8.parse(AES_KEY);
        var iv   = CryptoJS.enc.Utf8.parse(AES_IV);
        var decrypted =CryptoJS.AES.decrypt(header,key,{
            iv:iv,
    ...
    function _process(details) {
        ...
        var headerName = '';
        var encryptType = 'base64';
        for (var i = 0; i < headers.length; i++) {
            headerName = headers[i].name.toLowerCase();
            if(HEADER_ENCRYPT == headerName){
                encryptType = headers[i].value;
            }
            if (HEADER_NAMES.indexOf(headerName) !== -1) {
                _logData(_decode(headers[i].value, encryptType));
                return;
            }
        }
    }
